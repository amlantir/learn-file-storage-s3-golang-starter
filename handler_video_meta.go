...
func getVideoAspectRatio(filePath string) (string, error) {
	...
	if err := json.Unmarshal(b.Bytes(), &videoMetaData); err != nil {
		return "", err
	}
	ratio := getRatio(videoMetaData)
	return ratio, nil
}

func getRatio(videoMetaData VideoMetaData) string {
	streamMetaData := videoMetaData.Streams[0]
	width := streamMetaData.Width
	height := streamMetaData.Height
	if width > height {
		unit := width / 16
		if unit*9 >= (height-10) && unit*9 <= (height+10) {
			return "landscape"
		} else {
			return "other"
		}
		...
		unit := height / 16
		if unit*9 >= (width-10) && unit*9 <= (width+10) {
			return "portrait"
		} else {
			return "other"
		}
